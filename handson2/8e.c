// Name: Kantimahanty Rohit, Roll: MT2022149

// 8e : Write a separate program using signal system call to catch `SIGALRM` (use `setitimer` system call)

#include <sys/time.h>
#include <signal.h>
#include <unistd.h>
#include <stdio.h>

void callback()
{
    printf("Signal SIGALRM has been caught!\n");
    _exit(0);
}

void main()
{
    int timerStatus;                  // Determines success of `setitimer` call
    __sighandler_t status; // Determines status of `signal` call

    struct itimerval timer;

    // Generate signal after 4s
    timer.it_value.tv_sec = 4;
    timer.it_value.tv_usec = 0;
    timer.it_interval.tv_sec = 0;
    timer.it_interval.tv_usec = 0;

    timerStatus = setitimer(ITIMER_REAL, &timer, 0);
    if (timerStatus == -1)
        perror("\nError while setting an interval timer!");

    // Catch the SIGALRM signal
    status = signal(SIGALRM, (void *)callback);
    if (status == SIG_ERR)
        perror("\nError while catching signal!");
    else
        while (1);
}
